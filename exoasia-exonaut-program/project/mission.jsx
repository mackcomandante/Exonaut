...
function MissionDetail({ missionId, onBack, onSubmitted }) {
  ...
  // keep optimistic submitted flag in sync if store updates (e.g. lead grades it)
  React.useEffect(() => {
    setSubmitted(mySub?.state === 'pending' || mySub?.state === 'approved');
  }, [subs, mySub?.state]);

  const reviewerTrackCode = mission.track || currentProfile.trackCode || ME.track || 'AIS';
  const track = mission.track ? TRACKS.find(t => t.code === mission.track) : null;
  const reviewerTrack = TRACKS.find(t => t.code === reviewerTrackCode) || track || TRACKS[0];
  const activeCrown = (crownState.crowns || []).find(c => c.trackCode === reviewerTrackCode && c.status === 'active')
    || window.__crownStore?.getActiveCrownForTrack?.(reviewerTrackCode);
  const leadProfile = activeCrown
    ? profiles.find(p => p.id === activeCrown.userId)
    : null;
  const leadName = leadProfile?.fullName || leadProfile?.email || 'No active lead assigned';
  const leadSub = activeCrown
    ? `${reviewerTrack?.short || reviewerTrackCode} TRACK LEAD`
    : `${reviewerTrack?.short || reviewerTrackCode} TRACK`;
  const statusMap = {
  ...
  const submissionState = mySub?.state || (submitted ? 'pending' : null);
  const isPending = submissionState === 'pending';
  const isApproved = submissionState === 'approved';
  const st = submissionState === 'pending' ? statusMap.submitted : statusMap['not-started'];

  const handleFile = (f) => {
    if (!f) return;
    const tooLarge = f.size > 4 * 1024 * 1024;
    const nextFile = { name: f.name, size: (f.size / 1024).toFixed(1) + ' KB', type: f.type || 'application/octet-stream', dataUrl: '', reading: !tooLarge, tooLarge };
    setFile(nextFile);
    if (!tooLarge) {
      const reader = new FileReader();
      reader.onload = () => setFile(prev => prev && prev.name === f.name ? { ...prev, dataUrl: reader.result, reading: false } : prev);
      reader.onerror = () => setFile(prev => prev && prev.name === f.name ? { ...prev, reading: false } : prev);
  ...
  const handleSubmit = () => {
    if (file?.reading) {
      alert('The PDF is still loading. Try again in a moment.');
      return;
    }
    if (file?.tooLarge) {
      alert('This prototype can preview PDFs up to 4 MB. Use a smaller PDF so the Lead can open it.');
      return;
    }
    submitDeliverable({
  ...
  const effectivePoints = mySub?.pointsAwarded ?? 0;
  const effectiveStatus = submissionState === 'rejected'
    ? statusMap.rejected
    : submissionState === 'needs-revision'
    ? { cls: 'status-in-progress', label: 'NEEDS REVISION · RESUBMIT ONCE' }
    : submissionState === 'approved'
      ? statusMap.approved
      : st;
  const displayStatus = submissionState === 'needs-revision' ? statusMap['needs-revision'] : effectiveStatus;
      ...
      <div>
        ...
        <div className="mission-hero card-hud">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            ...
            <span className={'status-pill ' + displayStatus.cls}>{displayStatus.label}</span>
          </div>
        ...
        {effectiveFeedback && (
          <div className="card-panel" style={{
            borderColor: mySub?.state === 'rejected' ? 'rgba(239,68,68,0.3)' : mySub?.state === 'needs-revision' ? 'rgba(245,158,11,0.3)' : 'rgba(34,197,94,0.3)',
            background: mySub?.state === 'rejected' ? 'rgba(239,68,68,0.04)' : mySub?.state === 'needs-revision' ? 'rgba(245,158,11,0.04)' : 'rgba(34,197,94,0.04)',
            marginBottom: 20
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <i className="fa-solid fa-comment-dots" style={{ color: mySub?.state === 'rejected' ? 'var(--red)' : mySub?.state === 'needs-revision' ? 'var(--amber)' : 'var(--green)' }} />
              <span className="t-label" style={{ color: mySub?.state === 'rejected' ? 'var(--red)' : mySub?.state === 'needs-revision' ? 'var(--amber)' : 'var(--green)' }}>MISSION LEAD FEEDBACK</span>
            ...
            {mySub?.pointsAwarded != null && (
              <div className="t-mono" style={{ marginTop: 12, fontSize: 11, color: 'var(--ink)', letterSpacing: '0.1em' }}>
                +{mySub.pointsAwarded} PTS AWARDED
              </div>
            )}
          </div>
        )}

        {isApproved && !effectiveFeedback && (
          <div className="card-panel" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span className="t-label" style={{ color: 'var(--green)' }}>MISSION COMPLETED</span>
            </div>
            <div className="t-mono" style={{ fontSize: 12, color: 'var(--ink)', letterSpacing: '0.08em' }}>
              +{effectivePoints || mission.points} PTS AWARDED
            </div>
          </div>
        )}
        ...
        {!isApproved && (
          <div className="card-panel">
            <div className="section-head" style={{ marginBottom: 14 }}>
              <h2 style={{ fontSize: 16 }}>{isPending ? 'Your Submission' : 'Submit Deliverable'}</h2>
            ...
            {isPending ? (
              <div style={{ textAlign: 'center', padding: 24 }}>
                ...
                {mySub && !mySub.fileDataUrl && !mySub.filePath && mySub.state === 'pending' && (
                  <button className="btn btn-ghost btn-sm" style={{ marginTop: 14 }} onClick={() => { setSubmitted(false); setFile(null); }}>
                    <i className="fa-solid fa-upload" /> REPLACE FILE FOR LEAD PREVIEW
                  </button>
                )}
              ...
              <>
                {file ? (
                  <div className="file-attached">
                    ...
                    <div className="f-size">{file.tooLarge ? 'Too large for preview' : file.reading ? 'Loading preview...' : file.size}</div>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 20, justifyContent: 'flex-end' }}>
                  ...
                  <button className="btn btn-primary" disabled={!file || file.reading || file.tooLarge}
                    style={{ opacity: (file && !file.reading && !file.tooLarge) ? 1 : 0.4, cursor: (file && !file.reading && !file.tooLarge) ? 'pointer' : 'not-allowed' }}
      ...
      <div>
        ...
        <div className="card-panel">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <AvatarWithRing name={leadName} size={40} tier="corps" />
            <div>
              <div className="t-heading" style={{ fontSize: 13 }}>{leadName}</div>
      ...
      <div className="submit-inline-card graded">
        <div className="section-head" style={{ marginBottom: 10 }}>
          <h3 style={{ fontSize: 14, margin: 0 }}>
            ...
            {mySub?.state === 'rejected' ? 'Rejected' : mySub?.state === 'needs-revision' ? 'Needs Revision' : 'Graded · +' + effectivePoints + ' pts'}
          </h3>
  ...
  return (
    <div className="submit-inline-card">
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 14, justifyContent: 'flex-end' }}>
        ...
        <button className="btn btn-primary btn-sm" disabled={!file || file.reading || file.tooLarge}
          style={{ opacity: (file && !file.reading && !file.tooLarge) ? 1 : 0.4, cursor: (file && !file.reading && !file.tooLarge) ? 'pointer' : 'not-allowed' }}
          onClick={handleSubmit}>
          <i className="fa-solid fa-paper-plane" /> SUBMIT DELIVERABLE
        </button>
...
function XOCopilot({ mission }) {
  ...
  const onSend = () => {
    if (!input.trim() || busy || !prospect) return;
    const q = input.trim();
    setInput('');
