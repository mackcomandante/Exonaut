...
function AdminUsers() {
  ...
  const scopedUsers = scope === 'all' ? USERS : USERS.filter(u => getUserCohort(u.id) === scope);
  const scopeLabel = scope === 'all' ? 'All Cohorts' : (all.find(c => c.id === scope)?.name || '—');
  ...
  return (
    <div className="enter">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        <KPI label="TOTAL USERS" value={(scope === 'all' ? commanderCount + leadCount : 0) + scopedUsers.length} accent="sky" sub={scope === 'all' ? `${all.length} COHORTS` : 'IN SCOPE'} />
        <KPI label="COMMANDERS" value={scope === 'all' ? commanderCount : 0} accent="amber" sub="FOUNDER TIER" />
        <KPI label="MISSION LEADS" value={scope === 'all' ? leadCount : 0} accent="platinum" sub={`${(typeof TRACKS !== 'undefined') ? TRACKS.length : 7} TRACKS`} />
        <KPI label="EXONAUTS" value={scopedUsers.length} accent="lime" sub={scope === 'all' ? 'ACROSS ALL BATCHES' : 'IN COHORT'} />
      </div>
...
Object.assign(window, {
  ...
  AdminCohorts, AdminAssign, AdminUsers, AdminManagers,
});
